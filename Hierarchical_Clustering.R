...
#Record_3D_DF <- subset(Record_3D_DF, select = -c(Artist.Reputation, Base.Shipping.Price))
Record_3D_DF <- subset(Record_3D_DF, select = c(Height, Width))
head(Record_3D_DF)

Record_3D_DF <- sample_n(Record_3D_DF,20)
...
#labels(HClust_Ward_CosSim_N_3D) <- labels
plot(HClust_Ward_CosSim_N_3D, cex=.7, hang=-30,main = "Hierarchical Clustering using Manhattan Distance")
